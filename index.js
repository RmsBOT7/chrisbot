...
const config = {
    Akb4R: 'MILENABOT', // TOBAT SU ASU
    instagram: 'https://instagram.com/cy_chris.trash', // INFO JANGAN DI UBAH
    nomer: 'wa.me//5581996207906', // INFO SU JNGAN DI UBAH
    youtube: 'OFF', // KINTIL
    whatsapp: 'OFF', // BABI
...
client.sendMessage(from, drre, text, {quoted: mek})
		   ...
		   case 'marvelogo':
				...
				case 'qrcode':
					...
					const buffer = await getBuffer(`https://api.qrserver.com/v1/create-qr-code/?size=500x500&data=${tex}`)
					client.sendMessage(from, buffer, image, {quoted: mek})
					break
                                case 'wikien':
                ...
                case 'stiltext':
                      ...
                      gl1 = gh.split("|")[0];
                      gl2 = gh.split("|")[1];
                      buffer = await getBuffer(`https://api.vhtear.com/silktext?text=${gl1}&text2=${gl2}&apikey=ANTIGRATISNIHANJENKKK`)
                      reply(mess.wait)
                      client.sendMessage(from, buffer, image, {quoted: mek, caption: 'thund ni '+gh})
                      break
                case 'testing':
					var gh = body.slice(9)
					coli1 = gh.split("|")[0];
...
break
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buffer, image, {caption: `❮ 𝙋𝙀𝙎𝘼𝙉 𝘽𝙍𝙊𝘼𝘿𝘾𝘼𝙎𝙏 ❯\n\n${body.slice(4)}`})
						}
